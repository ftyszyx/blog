...
class Chentong(MyRequest):
    ...
    def download(self,path,url):
        if "dir" in url:
            ...
            dirjson=self._get(self._host_url+"/getdir.php?d=" + query + "&folder_id=" + "&passcode=""&r=" +str(random.random()) + "&ref=")
            dirjson=dirjson.json()
            ...
            filelist_json=filelist_json.json()
    ...
    def _parse_file(self,path,url):
        ...
        js_html = self._get(js_url).content.decode("utf-8")
        self._host_url = myhelp.getJsValue(js_html, "api_server")
        filejson = self._get(self._host_url + "/getfile.php?f=" + query +  "&passcode=""&r=" +str(random.random()) + "&ref=")
